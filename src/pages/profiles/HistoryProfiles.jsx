...
const HistoryProfiles = () => {
  ...
  const listData = profiles.data ? profiles.data : []
  ...
  return (
    <main className='w-screen h-screen flex flex-col justify-center items-center bg-gray-100'>
      <section className='flex flex-col justify-center items-center w-[1200px] h-full bg-gradient-to-t from-cyan-900 to-cyan-700 overflow-y-auto px-6 py-8'>
        <h1 className='text-white text-2xl mb-8'>Historial de Perfiles</h1>
          ...
          <table className='w-full text-white'>
            <thead>
              <tr className='border-b border-cyan-600'>
                ...
                <th className='text-left py-3 px-4 text-cyan-100'>Nombre del Perfil</th>
            ...
            <tbody>
              {
                Array.isArray(listData) && listData.map((item, index) => (
                  <tr key={item.id_profile} className='border-b border-cyan-700 hover:bg-cyan-700 transition-colors'>
                    <td className='py-3 px-4'>{index + 1}</td>
                    <td className='py-3 px-4'>{item.name_profile || 'N/A'}</td>
                    <td className='py-3 px-4'>
                      <div className='flex gap-2'>
                        <a
                          href={`/profiles/update/${item.id_profile}`}
                        ...
                        <a
                          href={`/profiles/delete/${item.id_profile}`}
...
export default HistoryProfiles
